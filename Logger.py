...
# ---------------------------------------------------------------------------
# Logging Helpers
# Thin wrappers around the standard logging module to produce uniform log
# entries that can be consumed by monitoring and alerting systems.
# ---------------------------------------------------------------------------
...
def logException(log, exception):
    logging.exception(
        f"{log.event} | {log.date} | C#EXCEPTION | {log.message} | {exception}"
    )


def example():
    # Sample usage for developers; not invoked in production code.
